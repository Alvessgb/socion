        ...
        <div className="flex flex-col gap-4">
          {matches.map((match) => {
            ...
            return (
              <div
                key={match.id}
                className="bg-white rounded-2xl border border-[#e4e4e7] p-5 flex items-center gap-5"
              >
                <Image
                  ...
                  width={56}
                  height={56}
                  className="rounded-xl border border-[#e4e4e7] flex-shrink-0"
                />
                <div className="flex-1 min-w-0">
                  <p className="text-[15px] font-[600] [font-feature-settings:'ss03'] text-black">
                    {other.name}
                  </p>
                  {otherProfile && (
                    <p className="text-[13px] font-[400] [font-feature-settings:'ss03'] text-[#71717a] mt-0.5 truncate">
                      {otherProfile.headline}
                    </p>
                  )}
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-[11px] font-[600] uppercase tracking-wide [font-feature-settings:'ss03'] text-[#a1a1aa]">
                      Match em {new Date(match.createdAt).toLocaleDateString("pt-BR")}
                    </span>
                    {hasPartnership && (
                      <span className="text-[11px] font-[600] bg-[#c1fbd4] text-[#16a34a] px-2 py-0.5 rounded-full">
                        Em negociação
                      </span>
                    )}
                  </div>
                ...
                <div className="flex flex-col gap-2 flex-shrink-0">
                  <Link
                    ...
                    className="px-4 py-2 rounded-full bg-[#f4f4f5] text-[13px] font-[500] [font-feature-settings:'ss03'] text-[#3f3f46] hover:bg-[#e4e4e7] transition-colors text-center"
                  >
                    Ver perfil
                  </Link>
                  {hasPartnership ? (
                    <Link
                      ...
                      className="px-4 py-2 rounded-full bg-black text-[13px] font-[500] [font-feature-settings:'ss03'] text-white hover:bg-[#1a1a1a] transition-colors text-center"
                    >
                      Parceria
                    </Link>
                  ) : (
...
function StartPartnershipButton({ matchId }: { matchId: string }) {
  ...
  return (
    <form action={startPartnership}>
      <button
        type="submit"
        className="w-full px-4 py-2 rounded-full bg-[#c1fbd4] text-[13px] font-[500] [font-feature-settings:'ss03'] text-[#16a34a] hover:bg-[#a8f5c2] transition-colors"
      >
        Iniciar parceria
      </button>
    </form>
